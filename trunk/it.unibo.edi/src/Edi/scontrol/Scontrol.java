...
//	 	private  IOutputView view;
		 ...
		 * perchË l'attesa di una request Ë bloccante, quindi, se fosse stato
...
//		}
	 ...
	 */
	// TODO:  vedi se c'Ë il modo di forzare la terminazione del thread in modo che l'evento non venga creato del tutto
	 ...
	 * e cambia lo stato dell'elettrodomestico in avvio (secondo la logica attuale la riattivazione Ë di fatto una accensione). 
	 * unica cosa in pi˘: riazzera il consumo dell'elettrodomestrico.
	 * NON PROVVEDE AD INVIARE LO STATO AGGIORNATO A SCONTROL
	 ...
	 * l'elettrodomestico da disattivare Ë quello acceso dopo tra quelli che consumano di pi˘.
	 * @return l'identificativo dell'elettrodomestico da riattivare
	 */
	private String stabilisciElettrodomesticoDaDisattivare (){
		// creo il peggiore elettrodomestico, acceso nel 1900 e con consumo pari al minor valore per un int: verr‡ per forza sovrescritto
	 ...
	 * l'elettrodomestico da riattivare Ë quello acceso prima tra quelli che consumano di meno.
	 * @return l'identificativo dell'elettrodomestico da riattivare
	 */
	private String stabilisciElettrodomesticoDaRiattivare (){
		// creo il peggiore elettrodomestico, acceso adesso e con consumo pari al maggior valore per un int: verr‡ per forza sovrescritto
	...
	/**
	 * valuta se Ë necessario disattivare un elettrodomestico ancora prima della sua accensione,
	 * questo puÚ capitare in un solo caso: tutti gli elettrodomestici in esercizio sono a basso consumo
	 ...
	 * @return
	 */
	private boolean valutaNecessit‡SpegnimentoPreventivo(){
	...
	/**
	 * determina se Ë necessario disattivare un elettrodomestico in quanto 
	 * si Ë superata la soglia di consumo
	 * @return true se il consumo degli elettrodomestici in esercizio Ë superiore
	 * alla soglia di consumo 
	 */
	private boolean valutaNecessit‡Disattivazione(){
	...
	/**
	 * Metodo che determina se Ë possibile riattivare un elettrodomestico precedentemente disattivato:
	 * in particolare guarda se c'Ë un elettrodomestico disattivato il cui consumo attuale ( che per gli 
	 * elettrodomestici disattivati Ë l'ultimo dato di consumo rilevato prima della disattivazione) 
	 * sommato al consumo attuale sia minore della soglia di consumo
	 * 
	 * @return true se Ë possibile riattivare un elettrodomestico
	 */
	private boolean valutaNecessit‡Riattivazione(){
		
		int margine = soglia - this.calcolaConsumoAttuale();
		/* ci si potrebbe mettere un controllo sul margine: con le specifiche attuali se Ë minore di 30
		 * di sicuro non si puÚ riattivare niente. perÚ questo lega troppo ai consumi dati dalle specifiche,
	 ...
	 * NOTA: c'Ë un po' di sporco dovuto al fatto che gli elettrodomestici disattivati consumano 0, ma hanno
	 ...
	 * puÚ essere superiore alla soglia
	...
	private void riceviEdElaboraComandoUserCmd(){
		try {
			...
			IStatus statusDaInviare=this.preparaStatus();// inizializzo lo status con quello attuale, ma tanto verr‡ sovrascritto, Ë giusto per non mettere null.
			if(comando.getComando()==ComandiUserCmd.connetti)
				 ...
				 *  
				 */
				if(this.valutaNecessit‡SpegnimentoPreventivo())
					statusDaInviare = this.preparaStatus("Impossibile accendere l'elettrodomestico "+comando.getIdElettrodomestico()+" tutta la potenza disponibile Ë gi‡ occupata da elettrodomestici a basso consumo non disattivabili");
				else{
				...
				if(this.valutaNecessit‡Riattivazione()){
	...
	try{
		...
		while(true){
			...
			if(messaggioSensore != null){
				...
				// se Ë la prima volta che ricevo i dati di consumo mando lo stato aggiornato a userCmd,
				// cosÏ l'utente sa quanto consuma l'elettrodomestico in fase di avvio. Se non lo facessi
				...
				// se l'elettrodomestico era in accensione ed invia un consumo pari alla met‡ delleo scorso vuol dire che 
				// Ë passato nella fase di esercizio. ciÚ significa che bisogna verificare se Ë necessario disattivare
				...
				if((elettrodomestico.getStato()== StatoElettrodomestico.avvio)&& datiSensore.getConsumoAttuale()== (elettrodomestico.getConsumo()/2)){
					...
					if(valutaNecessit‡Disattivazione()==true){
...
//	IMessage m2 = scontrolSense();
	...
	public void update(IResettableTimer safetyTimer, String idElettrodomestico) {
		...
		this.preparaStatus("non si ricevevano comunicazioni sul consumo da pi˘ di 2 secondi dall'elettrodomestico "+idElettrodomestico+". L'elettrodomestico Ë stato spento per ragioni di sicurezza");
	 ...
	 */
	public boolean valutaNecessit‡SpegnimentoPreventivoFT(){
		return this.valutaNecessit‡SpegnimentoPreventivo();
	}
	/**
	 * metodo che serve pre rompere l'incapsulamento col fine di verificare nei test il
	 * il risultato dell'omonimo metodo privato 
	 * @return il risultato del metodo omonimo privato
	 */
	public boolean valutaNecessit‡DisattivazioneFT(){
		return this.valutaNecessit‡Disattivazione();
	}
	/**
	 * metodo che serve pre rompere l'incapsulamento col fine di verificare nei test il
	 * il risultato dell'omonimo metodo privato 
	 * @return il risultato del metodo omonimo privato
	 */
	public boolean valutaNecessit‡RiattivazioneFT(){
		return this.valutaNecessit‡Riattivazione();
	}
	/**
	 * metodo che serve pre rompere l'incapsulamento col fine di verificare nei test il
	 * il risultato dell'omonimo metodo privato 
	...
	/**
	 * questo metodo permette di simulare ciÚ che accade quando Scontrol riceve i dati di consumo
	 * da un sensore.
